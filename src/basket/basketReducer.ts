...
export function BasketReducer(
    ...
    switch (action.type) {
        ...
        case "removeItem": {
            return {
                totalItemsNo: oldBasket.totalItemsNo - oldBasket.items[action.productIndex].productAmount,
                items: oldBasket.items.filter((e,i)=>i!==action.productIndex),
                totalPrice: oldBasket.totalPrice-oldBasket.items[action.productIndex].price
            };
